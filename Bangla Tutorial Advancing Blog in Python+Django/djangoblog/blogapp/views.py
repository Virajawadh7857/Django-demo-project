...
def getRegister(request):
    ...
    if form.is_valid():
        ...
        return HttpResponse("<h1> Thanks for your reg A confirmation link was send to your email")
...
def activate(request, uid, token):
    ...
    if user is not None and activation_token.check_token(user, token):
        user.is_active = True
        user.save()
        return HttpResponse("<h1> Account is activated. now you can <a href='/login'>login</h1>")
    else:
        return HttpResponse("<h1> Invalid Activation code... </h1>")
